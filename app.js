...
//const mongodb_URI = 'mongodb://localhost:27017/adamtzeng'
const mongodb_URI = 'mongodb+srv://cs_sj:BrandeisSpr22@cluster0.kgugl.mongodb.net/AdamTzeng?retryWrites=true&w=majority'
...
app.get('/upsertDB',
  async (req,res,next) => {
    ...
    for (book of books){
      const {author,country,language,link,pages,title}=book;
      await Course.findOneAndUpdate({author,country,language,link,pages,title},book,{upsert:true})
    }
...
app.post('/books/byAuthor', 
    async(req,res,next)=> {
      ...
      const books = await Course.find({author:regex}).sort({title:1})
      res.locals.books = books
      res.render('courselist')
    }
)

app.post('/books/byPages',
    async(req, res, next) => {
        const {pages} = req.body;
        const books = await Course.find({pages:pages}).sort({title:1})
        res.locals.books = books
        res.render('courselist')
    }
)
